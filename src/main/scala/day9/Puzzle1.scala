...
object Puzzle1 {
  ...
  class Computer(val memory: Array[Long],
                 val input: BlockingQueue[Long] = null,
                 val output: BlockingQueue[Long] = null,
                 val startingPC: Int = 0)
      extends Thread("Computer") {
    ...
    abstract class Op(val id: Short) {
      ...
      def getIndex(mode: Int, pcOffset: Int): Int = {
        if (mode == 1) pc + pcOffset
        else memory(pc + pcOffset).toInt + (if (mode == 2) relativeBase else 0)
      }

      def getIndexes(mode: Int): (Int, Int, Int) = {
        (
          getIndex(mode % 10, 1),
          getIndex((mode / 10) % 10, 2),
          getIndex((mode / 100) % 10, 3)
        )
      }
    }

    abstract class MathOp(override val id: Short,
                          val calc: (Long, Long) => Long)
        extends Op(id) {
    ...
    class JumpOp(override val id: Short, val f: Long => Boolean)
        extends Op(id) {
      override def doExec(mode: Short): Int = {
    ...
    object OutputOp extends Op(4) {
      override def doExec(mode: Short): Int = {
        val srcIdx: Int = getIndex(mode, 1)
    ...
    object LessThanOp
        extends MathOp(7, (val1: Long, val2: Long) => if (val1 < val2) 1 else 0)

    object EqualsOp
        extends MathOp(
          8,
          (val1: Long, val2: Long) => if (val1 == val2) 1 else 0
        )
    ...
    private val opMap: Map[Short, Op] = Set(
      AddOp,
      MultiplyOp,
      InputOp,
      OutputOp,
      ...
      EndOp
    ).map((op: Op) => (op.id, op)).toMap

    def execNext(): Unit = {
      val opMode: Long = memory(pc)
      val op: Op = opMap((opMode % 100).toShort)
      val mode: Short = (opMode / 100).toShort
      op.exec(mode)
    }
