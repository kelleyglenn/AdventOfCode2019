...
class Puzzle2Test extends AnyFlatSpec {
  behavior of "pointsWithDistanceAlongPath"
  it should "handle empty path" in {
    assert(Puzzle2.pointsWithDistanceAlongPath(Seq.empty, ((0, 0), 0)) == Map.empty)
  }

  it should "handle a single segment" in {
    val path = Seq((Direction.U, 3))
    val expected = Map(((0, 1), 1), ((0, 2), 2), ((0, 3), 3))
    assert(Puzzle2.pointsWithDistanceAlongPath(path, ((0, 0), 0)) == expected)
  }

  it should "handle two segments" in {
    val path = Seq((Direction.U, 3), (Direction.L, 2))
    val expected = Seq((0, 0), (0, 1), (0, 2), (0, 3), (-1, 3), (-2, 3)).zipWithIndex.toMap.removed((0, 0))
    assert(Puzzle2.pointsWithDistanceAlongPath(path, ((0, 0), 0)) == expected)
  }

  it should "handle a loop" in {
    val path = Seq((Direction.U, 3), (Direction.L, 2), (Direction.D, 2), (Direction.R, 3))
    val expected = Seq((0, 0), (0, 1), (0, 2), (0, 3),
      (-1, 3), (-2, 3),
      (-2, 2), (-2, 1),
      (-1, 1), (99, 99), (1, 1)).zipWithIndex.toMap.removed((0, 0)).removed((99, 99))
    assert(Puzzle2.pointsWithDistanceAlongPath(path, ((0, 0), 0)) == expected)
  }

  behavior of "sharedPointWithFewestSteps"
  it should "handle a single shared point" in {
    val line1 = Set((0, 0), (0, 1), (0, 2), (0, 3)).zipWithIndex.toMap.removed((0, 0))
    val line2 = Set((0, 0), (-1, 2), (0, 2), (1, 2)).zipWithIndex.toMap.removed((0, 0))
    assert(Puzzle2.sharedPointWithFewestSteps(line1, line2) == 4)
  }
  ...
  it should "handle puzzle example 1" in {
    ...
    assert(Puzzle2.distanceClosestSharedPointFromStrings(line1, line2) == 610)
  }
  ...
  it should "handle puzzle example 2" in {
    ...
    assert(Puzzle2.distanceClosestSharedPointFromStrings(line1, line2) == 410)
  }
  ...
  it should "solve the puzzle" in {
    ...
    assert(Puzzle2.distanceClosestSharedPointFromStrings(line1, line2) == 32132)
  }
